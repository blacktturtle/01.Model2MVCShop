...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<%for(int i=1;i<=totalPage;i++){
				...
				<a href="/listUser.do?page=<%=i%>&menu=search"><%=i %></a> <%--페이지 바꿀 시 다시 불러옴 --%>
				<%}else{ %> 
			<a href="/listUser.do?page=<%=i%>&menu=search&searchKeyword=<%=searchVO.getSearchKeyword() %>&searchCondition=<%=searchVO.getSearchCondition()%>"><%=i %></a> <%--페이지 바꿀 시 다시 불러옴 --%>
		<%
				}}
		%>	
    	</td>
